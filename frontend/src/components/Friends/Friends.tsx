...
const Friends: React.FC = () => {
  ...
  function goToProfile(otherUser: User) {
    navigate("/profile?user=" + otherUser.username);
  }
  ...
  useEffect(() => {
    ...
    socket.on("chat/refresh-friendships", () => {
      axios
        .get("http://" + hostname + ":10087/social", { withCredentials: true })
        .then((res) => {
          let friends: User[] = [];
  ...
  useEffect(() => {
    ...
    users.map((elem) => {
      if (!avatars.find((o) => o.id === elem.id)) {
        axios
          .get("http://" + hostname + ":10087/user/" + elem.id + "/avatar", {
          ...
          .then((res) => {
            setAvatars((oldArr) => [
              ...oldArr,
              { id: elem.id, avatar: URL.createObjectURL(res.data) },
            ]);
          })
          .catch((err) => {
            showSnackbarNotification(
  ...
  function acceptFriend(otherUser: User) {
    axios
      .put(
        "http://" + hostname + ":10087/social/" + otherUser.id + "/follow",
        {},
        { withCredentials: true }
      )
      .catch((err) => {
  ...
  function denyFriend(otherUser: User) {
    axios
      .delete(
        "http://" + hostname + ":10087/social/" + otherUser.id + "/unfollow",
        { withCredentials: true }
      )
      .catch((err) => {
        showSnackbarNotification(
          ...
          return (
            <>
              <div className="accept" onClick={() => acceptFriend(elem)}>
                <img src={require("../../images/coche.png")} />
              </div>
              <div className="denied" onClick={() => denyFriend(elem)}>
                <img src={require("../../images/declin.png")} />
              </div>
            </>
          );
        return (
          <>
            ...
            <div className="remove" onClick={() => denyFriend(elem)}>
              <img src={require("../../images/poubelle.png")} />
            </div>
          </>
        );
      }
    ...
    return users.map((elem) => {
      return (
        <div className="friendblock" key={elem.id}>
          ...
          <img
            className="avatar"
            src={avatars.find((o) => o.id === elem.id)?.avatar}
          />
          <div className="status">{getButtons(elem)}</div>
        </div>
      );
    });
  }
