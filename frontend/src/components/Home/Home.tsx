...
import "./style.css";
import TwoFA from "../Login/TwoFA/TwoFA"; // ADDED
...
const Home: FC = () => {
  ...
  useEffect(() => {
    axios.get("http://" + hostname + ":10087/auth/status", {
      withCredentials: true,
    }); // ADDED 
    // .catch((err) => {
    // 	if (err.data.message === 'username')
    // 		navigate('/login');
    // 	else if (err.data.message === 'twofa') // ADDED 2 lines
    // 		return (<TwoFA/>);
    // })
  }, []);
      ...
      >
        ...
        <li>
          <a
            onClick={() => {
              axios
                .get("http://" + hostname + ":10087/auth/logout", {
                  withCredentials: true,
                })
                .then(() => {
                  navigate("/login");
