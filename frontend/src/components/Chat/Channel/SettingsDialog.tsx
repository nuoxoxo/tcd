  ...
  const change = () => {
    ...
    if (!isProtected) {
      ...
      if (password.length < 3) {
        alert(
          "Protected group chat password must be at least 3 characters long"
        );
      }
      if (password.length > 20) {
        alert(
          "Protected group chat password cannot be longer then 20 characters"
        );
      }
    }
    ...
    <>
      {currentChat && (
        <Dialog open={open} onClose={setClose}>
          ...
          <DialogContent>
            ...
            {currentChat.type == "GROUP_PROTECTED" && (
              <TextField
                ...
                fullWidth
                onChange={(event) =>
                  setChannelPassword(event.currentTarget.value)
                }
                variant="standard"
              />
            )}
          ...
          <DialogActions>
            <Button color="secondary" variant="contained" onClick={setClose}>
              {" "}
              Cancel{" "}
            </Button>
            {currentChat.type == "GROUP_PROTECTED" ? (
