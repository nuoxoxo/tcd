...
const Channels: React.FC<{
  ...
  useEffect(() => {
    ...
    axios
      .get(
        "http://" + hostname + ":10087/chat/" + user.id + "/chats?filter=all",
  ...
  useEffect(() => {
    ...
    if (!currentChat && joinedChats.length > 0) {
      ...
      if (joinedChats.length != 1) setCurrentChat(joinedChats[0]);
      return;
    }
  ...
  function joinPublic(chat: Chat) {
    ...
    axios
      .patch(
        "http://" + hostname + ":10087/chat/" + user?.id + "/join",
        payload,
        { withCredentials: true }
      )
      .then((res) => {
        socket.emit("chat/join", {
          chatID: chat.id,
        }); /* Join the room / Let other users know that a new user joined */
        setRefreshChats(new Date().toISOString());
      })
      .catch((err) => {
        if (err.response.data.statusCode === 401) navigate("/login");
  ...
  const leaveChannel = (chatID: number) => {
    axios
      .delete(
        "http://" + hostname + ":10087/chat/" + user?.id + "/leave/" + chatID,
        { withCredentials: true }
      )
      .then((res) => {
  ...
  function renderChannelBox(chat: any, joined: boolean) {
    let selected;
    if (!currentChat) selected = false;
    else selected = currentChat.id == chat.id;

    if (chat.type === "PRIVATE") {
      return (
        <div
          className={selected ? "chat selected" : "chat"}
          key={chat.id}
          onClick={(event) => handleChatClick(chat, joined)}
        >
      ...
      return (
        ...
        >
          ...
          <p className="type">
            {chat.type == "GROUP_PROTECTED" ? "protected" : "public"}
          </p>
        </div>
      );
    }
  ...
  return (
    <div className="block channels">
      ...
      <div className="content">
        <button className="add" onClick={handlePublicDialogOpen}>
          Public
        </button>
        <button className="add" onClick={handleProtectedDialogOpen}>
          Protected
        </button>
