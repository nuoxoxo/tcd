...
export class Draw {
	...
	private basketballTheme: Basketball;
	private nightTheme: Night;
	private farmTheme: Farm;
	private beachTheme: Beach;
	...
	constructor(width: number, height: number) {
		...
		this.basketballTheme = new Basketball();
		this.farmTheme = new Farm();
		this.beachTheme = new Beach();

		this.footballTheme = new Football();		
		this.nightTheme = new Night();
	}
	...
	*/
	drawWaiting(theme: Theme, canvas: HTMLCanvasElement, context: CanvasRenderingContext2D) {
		...
		// }
		else if (theme === Theme.BASKETBALL) {
			this.basketballTheme.drawWaiting(canvas, context);
		}
		else if (theme === Theme.BEACH) {
			this.beachTheme.drawWaiting(canvas, context);
		}
		else if (theme === Theme.FARM) {
			this.farmTheme.drawWaiting(canvas, context);
		}
		else if (theme === Theme.NIGHT) {
			this.nightTheme.drawWaiting(canvas, context);
		}
	}
	...
	*/
	drawIntro(theme: Theme, i: number, socketID: string, dto: drawInterface) {
		...
		// }
			...
			this.basketballTheme.drawIntro(i, socketID, dto);
		}
		else if (theme === Theme.FARM) {
			this.farmTheme.drawIntro(i, socketID, dto);
		}
		else if (theme === Theme.BEACH) {
			this.beachTheme.drawIntro(i, socketID, dto);
		}
		else if (theme === Theme.NIGHT) {
			this.nightTheme.drawIntro(i, socketID, dto);
		}
	}
	...
	*/
	drawPlaying(theme: Theme, ball: Ball, dto: drawInterface) {
		...
		if (theme === Theme.CLASSIC) {
			...
			this.basketballTheme.drawPlaying(ball, dto);
		}
		else if (theme === Theme.FARM) {
			this.farmTheme.drawPlaying(ball, dto);
		}
		else if (theme === Theme.BEACH) {
			this.beachTheme.drawPlaying(ball, dto);
		} else if (theme === Theme.NIGHT) {
			this.nightTheme.drawPlaying(ball, dto);
		}
		// else if (theme === Theme.FOOTBALL) {
	...
	*/
	drawEnd(theme: Theme, i: number, winner: string, dto: drawInterface) {
		if (theme === Theme.CLASSIC) {
			...
			this.basketballTheme.drawEnd(winner, i, dto);
		}
		else if (theme === Theme.FARM) {
			this.farmTheme.drawEnd(winner, i, dto);
		}
		else if (theme === Theme.BEACH) {
			this.beachTheme.drawEnd(winner, i, dto);
		}
		// else if (theme === Theme.FOOTBALL) {
		// 	this.footballTheme.drawEnd(winner, i, dto);
		// }
		else if (theme === Theme.NIGHT) {
			this.nightTheme.drawEnd(winner, i, dto);
		}
	}
}
