...
const Social: FC = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://" + hostname + ":10087/user/all", { withCredentials: true })
      .then((res) => {
        setUsers(res.data);
      })
      .catch((err) => {
    ...
    users.map((elem) => {
      axios
        ...
        .then((res) => {
          ...
          setAvatars((prev) => [
            ...prev,
            { id: elem.id, avatar: imageObjectURL },
          ]);
        })
        .catch((err) => {
          if (err.response.data.statusCode === 401) navigate("/login");
      ...
      return (
        <div key={user.id} className="userblock">
          ...
          <div className="data">
            ...
            <h2 className="stats">
              {!user.wins && !user.loses ? "0" : user.wins / user.loses} W/L
            </h2>
          </div>
  ...
  function isAlready(isWhat: string, user: User): boolean {
    const friendship = friendships.find(
      (friendship) => friendship.user.id == user.id
    );
    if (friendship) return friendship.status == isWhat;
    return false;
  }
  function isSender(user: User): boolean {
    ...
    if (friendship) return friendship.sender.id == currentUser?.id;
    return false;
  }
    ...
    if (isUserSender && isPending) {
      return (
        <button
          className="follow"
          onClick={() => {
            handleUnfollow(enqueueSnackbar, user);
          }}
        >
          Cancel
        </button>
      ...
      return (
        <>
          ...
          <button
            className="follow accept"
            onClick={() => {
              handleFollow(enqueueSnackbar, user);
            }}
          >
            Accept
          </button>
